...
namespace Dorothy.Models
{
    ...
    public class UdpFlood : IDisposable
    {
            ...
            byte[][] pool, int wireSize, long targetBps, double tgtMbps)
        {
            ...
            Logger.Info($"[UDP] Single-threaded SendQueue loop: target={tgtMbps:F2} Mbps, " +
            ...
            var sendTask = Task.Run(() =>
            {
                ...
                var sender = new DoubleBufferedSender(
                    _device!, SendQueueCapacityBytes, FlushThresholdBytes, FlushThresholdPackets, "UDP");
                ...
                void EmitWorkerWindowIfDue()
                {
                    ...
                    double lastSendUs = sender.LastFlushTransmittedBytes > 0 && sender.LastFlushMicros > 0 && wireSize > 0
                        ...
                        : 0.0;
                    Logger.Info($"[UDP-W0] pkts={winPkts} bytes={winBytes} " +
                ...
                {
                    try { sender.Dispose(); }
                    catch (Exception ex) { Logger.Debug(ex, "[UDP] sender dispose failed"); }
                }
